...
the polymercylinder into a liquid media. The paper by Fu implements the 
...
# The LIGAND is free to diffuse along the vertical length (N) of the cylinder. 
# The host remains constant as it is part of the polymer matrix. The LIGAND can 
# reversibly bind to the the host to form a COMPLEX. Only the free LIGAND 
# diffuses into the liquid media and is called the RELEASE.
...
C_o = M_l / V_h                   # LIGAND in cylinder in mM
C_T = 0.008810573 / V_h           # Host concentration in cylinder in mM 
C_c = 0.00001 * C_T               # Fraction of hosts without bound drug

## Equilibrium concentrations
### LIGAND  + COMPLEX (LIGAND:host) = C_o
...
### Free LIGAND divided into the total number of layers, layer in direct contact 
### to media is set to 0
...
RELEASE   <- 0  # Initially there's no drug in the liquid environment
...
# At each time sum the concentration of free LIGAND and bound LIGAND (complex) within the 
# polymer to calculate the LIGAND depletion rate. I expect this to go down to 0.
...
tidy_release_polymer  <- as.data.frame(desolve_output) %>% 
  ...
  group_by(time) %>% 
  summarise(ligand_in_polymer = sum(concentration)) # Calculate the sum of free ligand and bound LIGAND(complex)
