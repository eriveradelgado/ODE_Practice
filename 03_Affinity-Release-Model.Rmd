...
p3 = C_T/C_o
 ...
 # RELEASE <- state[2*N+1]
  dLIGAND <- rep(0, times = N-1)
  dCOMPLEX <- rep(0, times = N-1)
...
dLIGAND[i] <- p2*(LIGAND[i + 1]-2*LIGAND[i] + LIGAND[i-1])/(delz^2) - Rb[i]}
    if(i == (N-1)){
dLIGAND[i] <-  p2*(-2*LIGAND[i]+LIGAND[i-1])/(delz^2) - Rb[i]}
...
M_l = .15 # Initial Loading concentration in millimoles.
